...
export default function EducatorEditGame() {
  ...
  const [logicPuzzleCount, setLogicPuzzleCount] = useState(0);
    ...
    const load = async () => {
      try {
        if (game.typeJeu === 'QUIZ') {
          ...
          if (!cancelled) {
            const puzzles = Array.isArray(res.data) ? res.data : [];
            setLogicPuzzleCount(puzzles.length);
          }
        }
      } catch (err: unknown) {
        if (!cancelled) {
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <label className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-700 font-semibold cursor-pointer hover:bg-emerald-100">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => void handleUploadCover(e.target.files?.[0])}
                      />
                    </label>
                    <button
                      type="button"
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-red-200 bg-red-50 text-red-700 font-semibold hover:bg-red-100 disabled:opacity-50"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    {game.typeJeu === 'LOGIQUE' && (
                      <div className="rounded-xl border border-gray-200 bg-white p-5">
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div>
                            <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Logique</p>
                            <p className="text-gray-900 font-semibold">{logicPuzzleCount} puzzle{logicPuzzleCount > 1 ? 's' : ''} configuré{logicPuzzleCount > 1 ? 's' : ''}</p>
                          </div>
                          {contentEditPath && (
                            <button
                              type="button"
