    ...
    Promise.all(
      games.map(async (g) => {
        try {
          ...
          if (g.typeJeu === 'LOGIQUE') {
            const res = await educatorApi.getLogicPuzzles(g.id);
            const puzzles = Array.isArray(res.data) ? res.data : [];
            return { id: g.id, hasContent: puzzles.length > 0 };
          }
                            ...
                            <div className="flex items-center justify-between gap-1">
                              ...
                              <div className="flex items-center gap-1">
                                {hasQuestionContent[game.id] && (
                                  ...
                                  >
                                    {submittingId === game.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Finaliser'}
                                  </button>
                                )}
                                <button
                                  type="button"
